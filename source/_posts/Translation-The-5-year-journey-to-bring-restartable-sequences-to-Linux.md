...
并发控制算法与per-CPU数据成对出现，是确保低级库和高性能应用程序在当今硬件上正确扩展不可或缺的一部分。这些算法保证在并发访问时，用户空间数据结构保持一致，并且用户数据的修改是执行完全的，以便于线程观察到的是之前或之后的状态，而不是中间状态。
...
但是，相互排斥不能很好地扩展，尤其是对于per-CPU数据。
由于在mutex加锁和解锁的临界区内只能有一个线程访问，因此可能存在大量线程在等待锁，而等待锁的时间内线程没有做任何其他的工作。
...
-->
接下来的可扩展的算法，原子比较和交换，在模型中，通常使用单个指令进行同步控制，例如`cmpxchg`或x86体系结构的lock前缀。
...
-->
但这里的问题是，在现代处理器上原子指令代价很高，x86的lock前缀很轻易地增加多个指令周期到没有前缀的相同指令的执行成本中。
更糟糕的是，无论是否只有一个线程在竞争，lock前缀都将会无条件执行。
...
-->
并不是所有体系结构都为原子化数据更新提供单独的指令。例如，ARM使用link-load/store-conditional（简称LL/SC）指令来读取原始数据、修改原始数据并写入新数据。
如果两个线程尝试同时写入相同的数据，则只有一个线程将成功，另一个线程将失败并重启LL/SC指令序列。
...
-->
Linux内核使用其他方法来保护per-CPU数据，例如禁用抢占或中断，或使用per-CPU本地的原子操作。不幸的是，这些机制要么不容易被用户空间使用，要么相对较慢（例如原子指令）。
所以，我们需要一种轻量型机制，用于在用户空间保护per-CPU数据，这是restartable sequences（RSEQ）的产生的动机。
...
RSEQ由新的系统调用`rseq(2)`所组成，该调用告诉内核当前线程的RSEQ的thread-local ABI（`sturct rseq`对象）在内存中的位置。此对象包含一个`rseq_cs`字段，该字段是指向当前被激活的RSEQ临界区的描述符（`sturct rseq_cs`对象）的指针，而在任何时候，只能由一个临界区被激活。
此ABI有两个用途：用户空间RSEQ和快速读取当前CPU编号。
...
-->
临界区可细分为准备阶段（preparatory stage）和提交阶段（commit stages），其中提交阶段是单个CPU的指令。
如果在提交阶段之前发生以下任何情况之一，则认为临界区当前被中断：
...
此外，由于线程被中断时需要回退机制，内核将instruction pointer指向中断处理程序的首地址，该处理程序需要执行一些纠正性措施，例如重新发起准备阶段。
在这种设计下，乐观情况（线程未中断）执行非常快，因为昂贵的`struct rseq_cs`对象的锁开销和原子指令可以完全被避免。
...
RSEQ的开始和结束由`start_ip`和`post_commit_ip`（指向提交指令后的指令）所表示，而`abort_ip`指向中断处理程序的首地址指令。
...
临界区和中断处理程序的实现都有所限制。例如，中止处理程序必须处于临界区以外；其次，在关键部分内不允许有系统调用；另外，尝试执行系统调用将导致进程发生segmentation fault而终止。
...
每当线程迁移到其他CPU上执行，且用户空间程序临界区的开始处读取`cpu_id_start`并比较这两个值时，内核就会更新`cpu_id`字段。
...
每当在执行当前`rseq_cs`描述符所描述的临界区范围之外的代码时，或发生抢断或传递信号时，就会将其指针设置为`NULL`。
...
Linux在4.18内核版本中合并了对RSEQ的支持，restartable sequences的概念最初是由[Paul Turner和Andrew Hunter在2013年所提出](https://blog.linuxplumbersconf.org/2013/ocw/system/presentations/1695/original/LPC%20-%20PerCpu%20Atomics.pdf)，其作为一种无需锁或开销较高的原子指令，即可从用户空间数据中安全访问per-CPU数据的方法。
但在当时还没有可用的patch。
...
在Paul发布该版本之后停了下来，Mathieu于2016年又重新开始，并提交了新的[patch集合](https://lwn.net/Articles/697756/)，并在[LPC 2016](https://blog.linuxplumbersconf.org/2016/ocw/system/presentations/3873/original/presentation-rseq-lpc2016.pdf)上介绍了这一集合。
他原本希望将Linux内核的4.15版补丁合并起来，但发现存在一个障碍。
...
虽然几乎每个版本的patch系列都有benchmark数据，但Linus明确表示，这种假设的用例[不足以](https://lwn.net/Articles/697991/)合并RSEQ的相关功能，并且需要具体的[性能数据](https://www.mail-archive.com/linux-kernel@vger.kernel.org/msg1213874.html)作为支撑。
...
Facebook提供了在jemalloc内存分配器上使用patch的[数据结果](https://lwn.net/Articles/661839/)。
因此，Mathieu收集了更多类似的基准结果，并在其他项目（如[LTTng-UST](https://github.com/lttng/lttng-ust)、[Userspace RCU](https://liburcu.org/)和[glibc](https://www.gnu.org/s/libc/)）上为RSEQ做好了准备。

最终，在最初开始的五年之后，该patch集合终于被合并到Linux内核中，Mathieu在Open Source Summit Europe 2018上作了名为[Improve Linux User-Space Core Libraries with Restartable Sequences](https://events.linuxfoundation.org/wp-content/uploads/2017/12/Improve-Linux-User-Space-Core-Libraries-with-Restartable-Sequences-Mathieu-Desnoyers-EfficiOS.pdf)的演讲，其中介绍了将RSEQ带入到Linux所作的多年努力。
...
使用RSEQ的首选方法是使用[librseq](https://github.com/compudj/librseq)库，该库提供了可能会用到的所有per-CPU操作，例如使`rseq(2)`对当前线程可用（`rseq_register_current_thread()`），查询当前线程的CPU编号（`rseq_current_cpu()`）和更新per-CPU数据（`rseq_cmpeqv_storev()`）。
...
使用`rseq(2)`需要以下两步。首先，需使用`rseq(2)`系统调用为当前线程启用该功能。
该系统调用具有以下的函数原型：
...
-->
假设你想使用`rseq(2)`来增加per-CPU计数器的值。为此，需要获取当前线程的CPU编号（存储在`struct rseq`的`cpu_id_start`字段中），并使用RSEQ修改per-CPU计数器的值。
这需要通过C和汇编混写的代码实现，下面是完成此操作的代码。
...
-->
`rseq_addv()`中的代码以`struct rseq_cs`对象填充作为开始，该对象描述了RSEQ中的字段，其中start符号为`1`，post-commit地址为`2`，中断处理程序为`4`。
如果线程未完成标签`1`和`2`之间的序列，那么将直接控制跳转到标签`4`，然后跳转到C中的`abort`标签。
...
RSEQ的主要使用场景之一是获取执行当前线程的CPU编号，这通常是指向per-CPU数据结构的索引值。现有使用`sched_getcpu()`来获取CPU编号的方法，在ARM上需进行系统调用，在 x86上需调用VDSO。而`rseq(2)`则程序直接允许读取内核和用户空间之间共享的`struct rseq`对象中缓存的CPU编号值。
...
下图演示了获取执行当前线程的CPU编号的RSEQ改进方法的相对速度提升。在下图中，值越小越好，其反映了速度的提升。
...
-->
如上所述，RSEQ适用于其他多种使用per-CPU数据的场景，其中之一是存储计数器值。下图显示了使用`rseq(2)`增加per-CPU计数器时，相对于使用`sched_getcpu()`和原子指令的速度提升。在ARM平台上显示有11倍的提升，而在x86显示是7.7倍提升。

{% asset_img rseq-arm32-percpu-stats.png  %} |  {% asset_img rseq-x86-64-percpu-stats.png  %}	
:-:|:-:
 在arm32上统计计数器增加benchmark(来自于 www.efficios.com) | 在x86_64上统计计数器增加benchmark(来自于 www.efficios.com)
<!-- 
...
LTTng-UST使用per-CPU的buffer来存储event。下图显示了使用`rseq(2)`增加per-CPU计数器时，相对于使用`sched_getcpu()`和原子指令的速度提升。在ARM平台上显示有1.1x的提升，而在x86显示是1.2x提升。

{% asset_img rseq-arm32-lttng-ust.png %} |  {% asset_img  rseq-x86-64-lttng-ust.png  %}
:-:|:-:
 在arm32的LTTng-UST上将event写入per-CPU trace的benchmark(来自于 www.efficios.com) |   在x86_64的LTTng-UST上将event写入per-CPU trace的benchmark(来自于 www.efficios.com)
...
LTTng-UST的问题有点不同：不更改线程的affinity mask，就无法在per-CPU数据结构之间移动数据。为了解决这个问题，Mathieu提出了一个新的[`cpu_opv`系统调用](https://lore.kernel.org/lkml/20181101095844.24462-1-mathieu.desnoyers@efficios.com/T/#u)，该调用在特定CPU上执行固定操作的向量（比较、memcpy 和add），类似于具有`readv(2)`和`writev(2)`的`struct iovec`概念。`cpu_opv`解决的`rseq(2)`的另一个问题是，如果单步执行到临界区，调试器将死循环，新的`cpu_opv`系统调用将允许调试器与现有应用程序合作，即使库使用了`rseq(2)`。
